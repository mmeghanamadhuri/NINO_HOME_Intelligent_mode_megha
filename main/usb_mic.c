...
static bool is_header_respeaker(uint8_t addr, uint16_t *vid_out, uint16_t *pid_out) {
  ...
  /* Accept Seeed ReSpeaker on GPIO-header FS root OR board USB / J18 hub.
   * Still require VID:PID 2886:0018 so the UVC camera is never opened as mic. */
  const bool ids_ok = read_usb_ids(hdl, &vid, &pid, NULL) &&
                      usb_host_device_info(hdl, &info) == ESP_OK &&
                      vid == RESPEAKER_VID && pid == RESPEAKER_PID;
  usb_host_device_close(s_uac_client, hdl);
  if (!ids_ok) {
    ESP_LOGW(TAG, "Ignore UAC addr=%u — expected ReSpeaker %04x:%04x",
             addr, RESPEAKER_VID, RESPEAKER_PID);
    return false;
  }
  ESP_LOGI(TAG, "ReSpeaker %04x:%04x at addr=%u speed=%s parent=%s",
           vid, pid, addr,
...
static void uac_host_lib_callback(uint8_t addr, uint8_t iface_num,
  ...
  ESP_LOGI(TAG, "UAC RX addr=%u iface=%u — queued (worker opens ReSpeaker 2886:0018 only)", addr,
           iface_num);
