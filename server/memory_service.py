...
class MemoryService:
    ...
    def run_daily_extraction_batch(self, day: date) -> dict[str, Any]:
        """Re-run long-term memory extraction on every turn from a calendar day."""
        if not self._ready:
            return {"skipped": True, "reason": "memory not ready"}
        if not SETTINGS.extraction_enabled:
            return {"skipped": True, "reason": "extraction disabled"}
        from memory_filters import should_extract_memories
        ...
        try:
            with self._connect() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        """
                        SELECT user_id, user_text, assistant_text
                        FROM conversations
                        WHERE timestamp::date = %s
            ...
            logger.warning("Daily extraction batch query failed: %s", exc)
            return {"ok": False, "error": str(exc)}

        processed = 0
        for user_id, user_text, assistant_text in rows:
            if not should_extract_memories(user_text):
                continue
            self._extract_memories_safe(user_id, user_text, assistant_text)
            processed += 1
    ...
    def export_training_day(self, day: date, dest_dir: Path) -> dict[str, Any]:
        """Write conversations + corrections for a day to JSONL training files."""
        dest = Path(dest_dir)
        dest.mkdir(parents=True, exist_ok=True)
        if not self._ready:
            return {"skipped": True, "reason": "memory not ready", "rows": 0}

        daily_path = dest / f"daily_{day.isoformat()}.jsonl"
        corpus_path = dest / "corpus.jsonl"
        rows_written = 0
        lines: list[str] = []
        ...
        try:
            with self._connect() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        """
                        SELECT c.user_id, u.name, c.user_text, c.assistant_text,
                               c.timestamp, c.session_id, COALESCE(c.corrected, FALSE)
                        FROM conversations c
                        JOIN users u ON u.id = c.user_id
                        WHERE c.timestamp::date = %s
                    ...
                    cur.execute(
                        """
                        SELECT cc.user_id, u.name, cc.correction_text,
                               cc.previous_assistant_text, cc.reworked_assistant_text,
                               cc.created_at
                        FROM conversation_corrections cc
                        JOIN users u ON u.id = cc.user_id
                        WHERE cc.created_at::date = %s
                    ...
                    for row in cur.fetchall():
                        record = {
                            ...
                            "user_id": int(row[0]),
                            "display_name": row[1],
                            "correction_text": row[2],
                            "previous_assistant_text": row[3],
                            "reworked_assistant_text": row[4],
                            "timestamp": row[5].isoformat() if row[5] else None,
                        }
                        lines.append(json.dumps(record, ensure_ascii=False))
                        rows_written += 1
        except Exception as exc:
            logger.warning("export_training_day failed: %s", exc)
    ...
    def get_last_conversation_turn(
        self, display_name: str, session_id: str = ""
    ) -> dict[str, Any] | None:
        """Most recent conversation row for rework (does not delete history)."""
        if not self.ready:
            return None
        ctx = self.ensure_user(display_name)
        if not ctx:
            return None
        try:
            with self._connect() as conn:
                with conn.cursor() as cur:
                    if session_id.strip():
                        cur.execute(
                            """
                            SELECT id, user_text, assistant_text, session_id
                            FROM conversations
                            WHERE user_id = %s AND session_id = %s
                            ORDER BY timestamp DESC
                            LIMIT 1
                            """,
                            (ctx.user_id, session_id.strip()),
                        )
                    else:
                        cur.execute(
                            """
            ...
            logger.warning("get_last_conversation_turn failed: %s", exc)
            return None

    def log_conversation_correction(
        self,
        display_name: str,
        *,
        correction_text: str,
        reworked_assistant_text: str,
        conversation_id: int | None = None,
    ) -> None:
        """Record a correction and append the reworked reply (original row kept)."""
        if not self.ready:
            return
        ctx = self.ensure_user(display_name)
        if not ctx:
        ...
        try:
            with self._connect() as conn:
                with conn.cursor() as cur:
                    ...
                    if conversation_id:
                        cur.execute(
                            "SELECT assistant_text FROM conversations WHERE id = %s",
                            (conversation_id,),
                        )
                        row = cur.fetchone()
                        if row:
                            prev_assistant = row[0]
                        cur.execute(
                            "UPDATE conversations SET corrected = TRUE WHERE id = %s",
                            (conversation_id,),
                        )
                    cur.execute(
                        """
                        INSERT INTO conversation_corrections
                            (conversation_id, user_id, correction_text,
                             previous_assistant_text, reworked_assistant_text)
                        VALUES (%s, %s, %s, %s, %s)
                        """,
                        (
                            conversation_id,
                    ...
                    cur.execute(
                        ...
                        (
                            ctx.user_id,
                            f"[correction] {correction_text.strip()}",
                            reworked_assistant_text.strip(),
                            None,
                        ),
                    )
            ...
            logger.warning("log_conversation_correction failed: %s", exc)
